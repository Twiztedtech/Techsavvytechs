...
export async function runOperationalHealthCheck({ notify = false, persist = false } = {}) {
  ...
  try {
    ...
    const data = qbo.data();
    const hasRefreshToken = Boolean(data?.encryptedRefreshToken || data?.refreshToken);
    checks.push({
      name: "quickbooks",
      status: qbo.exists && data?.realmId && hasRefreshToken ? "healthy" : "warning",
      message: qbo.exists && data?.realmId && hasRefreshToken ? "" : "QuickBooks connection needs attention.",
    });
  } catch (error) {
