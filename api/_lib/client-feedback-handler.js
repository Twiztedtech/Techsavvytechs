...
const list = (value, choices) =>
  Array.isArray(value)
    ? [...new Set(value.filter((item) => choices.includes(item)))].slice(
        0,
        choices.length,
      )
    : [];
...
export async function submitClientFeedback(req, res) {
  ...
  if (rateLimited(req, "client-feedback", 3, 60 * 60 * 1000))
    return res
      .status(429)
      .json({
        error: "Thank you. Please wait before submitting another response.",
      });
  ...
  try {
    ...
    if (required.some((field) => !clean(req.body?.[field], 3000)))
      return res
        .status(422)
        .json({ error: "Please complete the required questions." });
    const nps = Number(req.body?.nps);
    if (!Number.isInteger(nps) || nps < 0 || nps > 10)
      return res
        .status(422)
        .json({ error: "Choose a recommendation score from 0 to 10." });
    ...
    const response = {
      ...
      answers: {
        ...
        notificationHelpfulness: allowed(
          req.body.notificationHelpfulness,
          scale,
        ),
    ...
    if (
      !response.stage ||
      !response.answers.accountEase ||
      !response.answers.instructionsClear ||
      !response.answers.navigationEase ||
      !response.answers.satisfaction
    )
      return res
        .status(422)
        .json({ error: "One or more selections were not recognized." });
    ...
    await sendEmail({
      ...
      console.error("Client feedback notification failed:", error),
    );
    ...
    return res
      .status(500)
      .json({ error: "Your feedback could not be saved. Please try again." });
  }
}
