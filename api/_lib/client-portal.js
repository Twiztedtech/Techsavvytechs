...
export async function requireClient(req) {
  ...
  if (!profile.exists)
    throw Object.assign(new Error("Client membership is required."), {
      statusCode: 403,
    });
...
export function hasRole(profile, ...roles) {
  return (
    ...
    roles.some((role) => profile.roles.includes(role))
  );
}
...
export function publicTechnician(contractor, appointment = {}) {
  ...
  const displayName =
    contractor.publicDisplayName ||
    [parts[0], parts[1] ? `${parts[1][0]}.` : ""].filter(Boolean).join(" ");
  const directAllowed =
    appointment.directContactApproved === true &&
    contractor.allowDirectClientContact === true;
  return {
    displayName,
    profilePhotoUrl:
      contractor.showPhotoToClients === true
        ? clean(contractor.profilePhotoUrl, 1000)
        : "",
...
export async function sendSms({
  ...
  if (!phone || !sid || !token || (!serviceSid && !from))
    return { skipped: true };
  ...
  if (process.env.APP_URL)
    params.set(
      "StatusCallback",
      `${process.env.APP_URL.replace(/\/$/, "")}/api/webhooks/twilio`,
    );
...
export async function notifyNewRequest(request) {
  ...
  await Promise.allSettled([
    sendEmail({
      to: emails,
      subject: `${request.urgent ? "URGENT: " : ""}New client job request ${request.requestNumber}`,
      text: `${summary}\n\n${reviewUrl}`,
      html: `<h1>New client job request</h1><p>${escapeHtml(summary)}</p><p><a href="${reviewUrl}">Review request</a></p>`,
      type: "new_request",
    }),
    ...phones.map((phone) =>
      sendSms({
        to: phone,
        body: `New ${request.urgent ? "URGENT " : ""}job request ${summary}. Review: ${reviewUrl}`,
        type: "new_request",
        important: true,
      }),
    ),
...
export async function uploadInlineFiles(files, requestId) {
  ...
  for (const file of files) {
    ...
    if (!bytes || bytes.length > 1_500_000 || !allowed.has(contentType))
      throw Object.assign(
        new Error(
          "Attachments must be PDF, JPG, PNG, or text files smaller than 1.5 MB.",
        ),
        { statusCode: 422 },
      );
    ...
    await adminStorage
      .file(path)
      .save(bytes, { metadata: { contentType }, resumable: false });
    uploaded.push({
      name: safeName,
      storagePath: path,
      contentType,
...
export function encryptSecret(value) {
  const key = createHash("sha256")
    .update(process.env.CLIENT_PORTAL_SECRET || "")
    .digest();
...
export async function googleAccessToken() {
  ...
  if (!refreshToken) {
    const connection = await adminDb
      ...
      .doc("google_calendar")
      .get();
    refreshToken = connection.exists
      ? decryptSecret(connection.data().encryptedRefreshToken)
      : "";
  }
  if (
    !refreshToken ||
...
export async function renewGoogleCalendarWatch() {
  ...
  const address = process.env.APP_URL
    ? `${process.env.APP_URL.replace(/\/$/, "")}/api/webhooks/google-calendar`
    : "";
  if (
    !accessToken ||
    !calendarId ||
  ...
  if (!response.ok)
    throw new Error(
      "Google Calendar notification channel could not be created.",
    );
  await adminDb
    .collection("settings")
    .doc("google_calendar")
    .set(
      {
        watchChannelIdHash: hashValue(channelId),
        watchResourceId: data.resourceId || "",
        watchExpiresAt: new Date(
          Number(data.expiration || expiration),
        ).toISOString(),
        syncPending: true,
        updatedAt: nowIso(),
      },
...
export async function syncGoogleCalendarChanges() {
  ...
  do {
    ...
    if (settings.data()?.syncToken)
      url.searchParams.set("syncToken", settings.data().syncToken);
    else
    ...
    if (response.status === 410) {
      await settingsRef.set(
        { syncToken: "", syncPending: true },
        { merge: true },
      );
      return syncGoogleCalendarChanges();
    }
    const data = await response.json();
    if (!response.ok)
      throw new Error("Google Calendar changes could not be synchronized.");
    for (const event of data.items || []) {
      const appointmentId =
        event.extendedProperties?.private?.techsavvyAppointmentId;
      ...
      if (event.status === "cancelled")
        await ref.set(
          {
            status: "cancelled",
            calendarUpdatedAt: event.updated || nowIso(),
            updatedAt: nowIso(),
          },
          { merge: true },
        );
        ...
        event.end?.dateTime &&
        (appointment.data().confirmedStart !== event.start.dateTime ||
          appointment.data().confirmedEnd !== event.end.dateTime) &&
        appointment.data().calendarUpdatedAt !== event.updated
      ) {
        await ref.set(
          {
  ...
  await settingsRef.set(
    { syncToken: nextSyncToken, syncPending: false, lastSyncedAt: nowIso() },
    { merge: true },
  );
  return { updated };
}

export async function syncCalendarAppointment(
  appointment,
  job,
  existingEventId = "",
) {
  ...
  const encodedEvent = existingEventId
    ? `/${encodeURIComponent(existingEventId)}`
    : "";
  const response = await fetch(
    `https://www.googleapis.com/calendar/v3/calendars/${encodedCalendar}/events${encodedEvent}`,
    {
      ...
      body: JSON.stringify({
        ...
        start: {
          dateTime: appointment.confirmedStart,
          timeZone: "America/Los_Angeles",
        },
        end: {
          dateTime: appointment.confirmedEnd,
          timeZone: "America/Los_Angeles",
        },
        extendedProperties: {
          private: {
            techsavvyAppointmentId: appointment.id,
            techsavvyJobId: appointment.jobId,
          },
        },
      }),
    },
  ...
  if (!response.ok)
    throw new Error(
      `Google Calendar rejected the appointment (${response.status}).`,
    );
