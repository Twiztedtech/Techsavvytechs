...
function renderCustomerDocumentEmail({ type, document, number, total, link, expiresAt, hasPdf }) {
  ...
  const action = type === "quote" ? "Review and approve quote" : "View & pay invoice";
  ...
  return {
    ...
    html: `<div style="background:#f1f5f2;padding:20px 0"><div style="font-family:Arial,sans-serif;max-width:620px;margin:0 auto;color:#17201a;line-height:1.55;background:#ffffff"><div style="background:#000000;text-align:center"><img src="${appUrl}/logo-email.png" width="420" alt="TechSavvy LLC — Wired for What's Next." style="display:block;margin:0 auto;max-width:100%;height:auto"></div><div style="height:4px;background:#66dc14"></div><div style="padding:28px;border:1px solid #e2e8f0;border-top:0"><p style="margin-top:0">${type === "quote" ? "A quote is ready for your review and approval." : "Your invoice is ready to view and pay."}</p><table role="presentation" width="100%" style="border-collapse:collapse"><tr><td style="background:#66dc14;padding:14px 16px;font-size:13px;font-weight:700;color:#000">${escapeHtml(number)}</td><td align="right" style="background:#66dc14;padding:14px 16px;font-size:22px;font-weight:700;color:#000">${escapeHtml(total)}</td></tr></table>${apDetailsHtml}<p style="margin-top:22px;text-align:center"><a href="${escapeHtml(link)}" style="display:inline-block;background:#000000;color:#66dc14;padding:14px 26px;border-radius:5px;text-decoration:none;font-weight:700;border:2px solid #66dc14">${action}</a></p>${pdfNoteHtml}<p style="font-size:12px;color:#64748b">This secure link expires ${escapeHtml(expiresAt.slice(0, 10))}. If you have questions, reply to this email.</p></div><div style="background:#000000;color:#c8cdc9;font-size:11px;text-align:center;padding:12px">TechSavvy LLC &nbsp;|&nbsp; Fairfield, CA &nbsp;|&nbsp; (707) 653-6702 &nbsp;|&nbsp; techsavvytechs.com</div></div></div>`,
  };
}
