...
- Hourly appointment, closeout, and calendar automation is live through the Cloudflare Worker in [`cloudflare/portal-scheduler`](cloudflare/portal-scheduler). Its `0 * * * *` trigger is managed in source control, uses an encrypted shared secret, and replaces only the former Vercel client-portal cron. The consolidated daily CRM reminder and reconciliation job remains on Vercel.
...
- **Scheduling automation:** [`cloudflare/portal-scheduler`](cloudflare/portal-scheduler) invokes `/api/cron/client-portal` at the top of every hour (UTC) with a shared `CRON_SECRET`. Cloudflare and Vercel store the value as encrypted secrets. The former Vercel client-portal cron has been removed to prevent duplicate runs; the separate consolidated CRM reminder and reconciliation cron remains enabled.
