...
type ReminderDelivery = { id: string; type: string; entityId: string; email: string; status: string; manual?: boolean; sentAt?: string; createdAt?: string; error?: string; escalation?: "gentle" | "firm" | "urgent" };
    ...
    <div className={`min-h-screen bg-crm-surface-soft text-crm-body ${crmTheme === "dark" ? "dark" : ""}`}>
      <header className="sticky top-0 z-40 flex h-16 items-center border-b border-crm-hairline bg-crm-canvas px-3 text-crm-ink lg:px-5">
        ...
        <div className="flex items-center gap-3 border-r border-crm-hairline pr-4">
          <img
            src={crmTheme === "dark" ? "/logo-dark.png" : "/logo-light.png"}
            alt="TechSavvy LLC"
...
function RemindersView({ customers, jobs, quotes, invoices, assets, deliveries }: { customers: LiveCustomer[]; jobs: LiveJob[]; quotes: LiveQuote[]; invoices: LiveInvoice[]; assets: CustomerAsset[]; deliveries: ReminderDelivery[] }) {
  ...
  const items = [
    ...
    ...invoices.filter((invoice) => invoice.balance > 0 && invoice.dueDate && new Date(`${invoice.dueDate}T00:00:00`) < today).map((invoice) => {
      const daysOverdue = Math.max(0, Math.floor((today.getTime() - new Date(`${invoice.dueDate}T00:00:00`).getTime()) / 86400000));
      const stage = daysOverdue >= 30 ? "urgent" : daysOverdue >= 14 ? "firm" : "gentle";
      return { type: "invoice", id: invoice.id, title: invoice.invoiceNumber || invoice.id, detail: `${invoice.customer} · ${invoice.balance.toLocaleString(undefined, { style: "currency", currency: "USD" })} overdue · ${daysOverdue}d overdue · ${stage}`, stage };
    }),
