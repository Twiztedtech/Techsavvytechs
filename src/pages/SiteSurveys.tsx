...
function EstimateHandoff({ survey, user, onConverted }: { survey: SiteSurvey; user: User; onConverted: () => Promise<void> }) {
  ...
  return <section className="mt-8 rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-bold text-white">Estimate handoff</h3><p className="mt-1 text-sm text-slate-400">Match survey SKUs to catalog pricing, review the BOM, then create a CRM draft.</p></div>{!plan && <button disabled={!eligible || busy} onClick={prepare} className="rounded-xl bg-amber-500 px-4 py-2 text-sm font-black text-slate-950 disabled:opacity-40">{busy ? 'Preparing…' : 'Prepare estimate'}</button>}</div>{!eligible && <p className="mt-4 rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm text-slate-400">Approve the survey before creating an estimate.</p>}{plan && <><Field label="Estimate title" value={title} onChange={setTitle} /><div className="mt-4 grid gap-3">{items.map((item, index) => <div key={`${item.sku}-${index}`} className="rounded-xl border border-slate-700 bg-slate-950 p-3"><div className="mb-3 flex items-start justify-between gap-3"><div>{item.kind === 'labor' ? <><p className="text-sm font-bold text-sky-300">Labor</p><p className={`mt-1 text-xs ${item.unitPrice > 0 ? 'text-slate-400' : 'text-amber-200'}`}>{item.unitPrice > 0 ? 'Hours are an allowance based on the survey. Adjust as needed.' : 'No agreed labor rate for this customer. Enter the hourly rate.'}</p></> : <><p className="font-mono text-sm font-bold text-amber-300">{item.sku || 'CUSTOM'}</p><p className={`mt-1 text-xs ${item.catalogMatched ? 'text-emerald-300' : 'text-amber-200'}`}>{item.catalogMatched ? 'Catalog price matched' : 'No catalog match — enter price manually'}</p></>}</div><button onClick={() => remove(index)} aria-label={`Remove ${item.description}`} className="text-red-300"><Trash2 size={17} /></button></div><div className="grid gap-3 sm:grid-cols-[minmax(0,2fr)_110px_130px]"><Field label="Description" value={item.description} onChange={(value) => update(index, { description: value })} /><Field label={item.kind === 'labor' ? 'Hours' : 'Quantity'} type="number" value={item.quantity} onChange={(value) => update(index, { quantity: Number(value) })} /><Field label={item.kind === 'labor' ? 'Rate ($/hr)' : 'Unit price'} type="number" value={item.unitPrice} onChange={(value) => update(index, { unitPrice: Number(value) })} /></div></div>)}</div><div className="mt-4 flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs uppercase tracking-wider text-slate-500">Draft total</p><p className="text-2xl font-black text-white">{total.toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</p></div><button disabled={busy || !items.length || items.some((item) => !item.description || item.quantity <= 0)} onClick={convert} className="rounded-xl bg-emerald-500 px-5 py-3 font-black text-emerald-950 disabled:opacity-40">{busy ? 'Creating…' : 'Create draft estimate'}</button></div></>}{error && <p className="mt-4 text-sm text-red-300">{error}</p>}</section>;
}
