...
function RecordEditor({ moduleKey, fields, record, onSave, onDelete }: { moduleKey: string; fields?: SurveyFieldDefinition[]; record: SurveyRecord; onSave: (record: SurveyRecord) => Promise<void>; onDelete: () => Promise<void> }) {
...
function SurveyEditor({ user, initial, canReview, canEstimate }: { user: User; initial: SiteSurvey; canReview: boolean; canEstimate: boolean }) {
  ...
  async function setStatus(status: SurveyStatus) { setMessage(''); try { setSurvey(await surveyRequest<SiteSurvey>(user, '', { action: 'changeStatus', id: survey.id, status })); } catch (reason) { setMessage(reason instanceof Error ? reason.message : 'Status update failed.'); } }
  async function uploadPhoto(file?: File) { if (!file) return; setMessage('Compressing photo…'); try { const dataUrl = await compressSurveyPhoto(file); await surveyRequest(user, '', { action: 'uploadAttachment', id: survey.id, moduleKey: active, dataUrl, caption: file.name }); setMessage('Photo uploaded'); } catch (reason) { setMessage(reason instanceof Error ? reason.message : 'Photo upload failed.'); } }
  ...
  return <div className="mx-auto max-w-7xl px-3 py-5 sm:px-6 sm:py-8">
    ...
    <div className="grid gap-5 lg:grid-cols-[240px_minmax(0,1fr)]"><nav className="flex gap-2 overflow-x-auto lg:grid lg:content-start">
      {survey.moduleKeys.map((key) => <button key={key} onClick={() => setActive(key)} className={`whitespace-nowrap rounded-xl px-4 py-3 text-left text-sm font-bold ${active === key ? 'bg-amber-500 text-slate-950' : 'border border-slate-800 bg-slate-900 text-slate-300'}`}>{survey.modules?.find((module) => module.id === key)?.definition.title || key}</button>)}
      <button onClick={() => setActive('review')} className={`whitespace-nowrap rounded-xl px-4 py-3 text-left text-sm font-bold ${active === 'review' ? 'bg-amber-500 text-slate-950' : 'border border-slate-800 bg-slate-900 text-slate-300'}`}>Review & BOM</button>
