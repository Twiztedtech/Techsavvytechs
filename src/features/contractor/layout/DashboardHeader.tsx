...
export function DashboardHeader({ role, onRoleChange, onContactAdmin, onSignOut }: DashboardHeaderProps) {
  return (
      ...
      <div className="max-w-7xl mx-auto px-4 py-3 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 text-xs">
          <div className="bg-slate-950 border border-slate-800 rounded p-1 flex items-center gap-1">
            <button type="button" onClick={() => onRoleChange('contractor')} className={`px-3 py-1 rounded text-xs font-bold transition ${role === 'contractor' ? 'bg-amber-500 text-slate-950' : 'text-slate-400 hover:text-white'}`}>Contractor Mode</button>
