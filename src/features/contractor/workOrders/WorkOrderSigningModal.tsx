      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {hasSignature && (
          <button
            type="button"
            onClick={clear}
            className="text-[10px] font-bold text-green-700 hover:text-green-900 underline"
          >
            Clear
          </button>
        )}
...
export function WorkOrderSigningModal({
  job,
  technicianName,
  savedTechnicianSignature = "",
  onSaveTechnicianSignature,
  onClose,
  onComplete,
}: Props) {
  ...
  const equipment =
    job.equipment?.filter((item) => item.description.trim()) ?? [];
  ...
  const [useSavedTechnicianSignature, setUseSavedTechnicianSignature] =
    useState(Boolean(savedTechnicianSignature));
  const [saveTechnicianSignature, setSaveTechnicianSignature] = useState(
    !savedTechnicianSignature && Boolean(onSaveTechnicianSignature),
  );
  ...
  const complete = async () => {
    if (
      ...
      !customerSignature ||
      !customerAccepted ||
      checks.some((checked) => !checked)
    ) {
      alert(
        "Complete the checklist, enter both names, capture both signatures, and confirm customer acceptance.",
      );
      return;
    }
    ...
    try {
      ...
      const ensureSpace = (height: number) => {
        ...
        document.text(
          `TECHSAVVY FIELD WORK ORDER · ${job.workOrderNumber || job.id}`,
          margin,
          y,
        );
        y += 28;
      ...
      const scopeEntries = scopeTasks.length
        ? scopeTasks
        : [job.notes || "Scope confirmed at the job site."];
      scopeEntries.forEach((task, index) => {
        const scopeLines = document.splitTextToSize(
          `${index + 1}. ${task}`,
          510,
        );
      ...
      document.text(
        `Signed electronically on ${new Date(completedAt).toLocaleString()}.`,
        margin,
        y,
      );
      y += 14;
      document.text(
        "The site representative confirms that the completed work was reviewed and accepted.",
        margin,
        y,
      );
      y += 16;
      ...
      const safeNumber = (job.workOrderNumber || job.id).replace(
        /[^a-zA-Z0-9_-]/g,
        "-",
      );
      ...
      const fileRef = ref(
        storage,
        `signed-work-orders/${job.id}/${Date.now()}-${fileName}`,
      );
      ...
      if (!response.ok)
        throw new Error(
          (await response.json()).error ||
            "Could not save the signed work order.",
        );
      alert(
        "Signed work order saved. You can open the PDF from this job at any time.",
      );
      onComplete();
    } catch (error) {
      ...
      alert(
        error instanceof Error
          ? error.message
          : "The signed work order could not be saved.",
      );
    } finally {
      setIsSaving(false);
    }
    ...
    <div className="fixed inset-0 z-[100] overflow-y-auto bg-slate-950/85 p-3 sm:p-6">
      <div className="mx-auto my-4 max-w-2xl overflow-hidden rounded-2xl border border-green-500/40 bg-white shadow-2xl">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-green-400">
                TechSavvy Field Service
              </p>
              <h2 className="mt-1 text-lg font-black text-white">
                Complete & Sign Work Order
              </h2>
              <p className="mt-1 text-xs text-slate-400">
                {job.workOrderNumber || job.id} · {job.name}
              </p>
            </div>
            <button
              type="button"
        ...
        <div className="space-y-5 p-5 text-slate-900 sm:p-7">
          <div className="rounded-lg border border-green-200 bg-green-50 p-3 text-xs text-slate-700">
            <strong className="text-green-800">Scope:</strong>
            {scopeTasks.length ? (
              ...
              <span> {job.notes || "Scope confirmed at the job site."}</span>
            )}
          </div>
          {equipment.length ? (
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700">
              <p className="font-bold text-slate-900">Equipment & materials</p>
          ...
          <div className="space-y-2">
            <p className="text-sm font-bold">Completion checklist</p>
          ...
          {useSavedTechnicianSignature && savedTechnicianSignature ? (
            <div className="rounded border border-green-200 bg-green-50 p-3">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-bold text-green-900">
                    Saved technician signature
                  </p>
                  <p className="mt-0.5 text-[10px] text-green-800">
                    This signature will be applied to this work order.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => {
                    setUseSavedTechnicianSignature(false);
                    setTechnicianSignature("");
                  }}
                  className="rounded border border-green-700 px-2.5 py-1.5 text-[10px] font-bold text-green-800 hover:bg-green-100"
                >
                  Replace
                </button>
              </div>
              <img
                src={savedTechnicianSignature}
                alt="Saved technician signature"
                className="mt-3 h-20 w-full rounded border border-green-200 bg-white object-contain"
              />
            </div>
          ) : (
            <>
              ...
              {onSaveTechnicianSignature ? (
                <label className="flex items-center gap-2 text-xs text-slate-700">
                  <input
                    type="checkbox"
                    checked={saveTechnicianSignature}
                    onChange={(event) =>
                      setSaveTechnicianSignature(event.target.checked)
                    }
                    className="h-4 w-4 accent-green-600"
                  />
                  Save this as my signature for future work orders
                </label>
              ) : null}
            </>
          )}
          ...
          />
          <label className="flex gap-2 rounded border border-green-200 bg-green-50 p-3 text-xs text-slate-700">
            <input
              type="checkbox"
            ...
            />
            <span>
              I confirm that I am authorized to accept this work on behalf of
              the site and that the scope above has been reviewed.
            </span>
          </label>
          <div className="flex flex-col-reverse gap-2 border-t border-slate-200 pt-4 sm:flex-row sm:justify-end">
            <button
              ...
              onClick={onClose}
              disabled={isSaving}
              className="rounded px-4 py-2 text-sm font-bold text-slate-600 hover:bg-slate-100"
            >
              Cancel
            </button>
            <button
              ...
              onClick={complete}
              disabled={isSaving}
              className="rounded bg-green-600 px-4 py-2 text-sm font-bold text-white hover:bg-green-700 disabled:cursor-wait disabled:opacity-60"
            >
              {isSaving
                ? "Generating signed PDF…"
                : "Generate & Save Signed PDF"}
            </button>
          </div>
        </div>
      </div>
